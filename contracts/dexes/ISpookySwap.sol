...
interface ISpookySwapFactory {
    function getPair(IERC20 tokenA, IERC20 tokenB) external view returns (ISpookySwapPair pair);
}
...
library ISpookySwapPairExtension {
    ...
    function doCalculate(
        ...
        // Uniswap V2 now requires fixed 0.2% swap fee
...
library ISpookySwapFactoryExtension {
    using SafeMath for uint256;
    using UniversalERC20 for IERC20;
    using ISpookySwapPairExtension for ISpookySwapPair;
    ...
    function calculateSwapReturn(
        ...
        ISpookySwapPair pair = factory.getPair(realInToken, realOutToken);
        if (pair != ISpookySwapPair(0)) {

...
/**
 * @notice https://github.com/Layer3Org/spiritswap-core/
 */
interface ISpiritSwapFactory {
    function getPair(IERC20 tokenA, IERC20 tokenB) external view returns (ISpiritSwapPair pair);
}

/**
 * @notice Uniswap V2 pair pool interface. See https://uniswap.org/docs/v2/smart-contracts/pair/
 */
interface ISpiritSwapPair {
    function swap(
        uint256 amount0Out,
        uint256 amount1Out,
        address to,
...
library ISpiritSwapPairExtension {
...
library ISpiritSwapFactoryExtension {
    using SafeMath for uint256;
    using UniversalERC20 for IERC20;
    using ISpiritSwapPairExtension for ISpiritSwapPair;
    ...
    function calculateSwapReturn(
        ...
        ISpiritSwapPair pair = factory.getPair(realInToken, realOutToken);
        if (pair != ISpiritSwapPair(0)) {

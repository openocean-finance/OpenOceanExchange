...
/**
 * @notice https://github.com/babyswap/baby-swap-contract/blob/main/contracts/swap/BabyFactory.sol
 */
interface IBabyswapFactory {
    function getPair(IERC20 tokenA, IERC20 tokenB) external view returns (IBabyswapPair pair);
}

/**
 * @notice https://github.com/babyswap/baby-swap-contract/blob/main/contracts/swap/BabyPair.sol
 */
interface IBabyswapPair {
    function swap(
        uint256 amount0Out,
        uint256 amount1Out,
        address to,
...
library IBabyswapPairExtension {
...
library IBabyswapFactoryExtension {
    using SafeMath for uint256;
    using UniversalERC20 for IERC20;
    using IBabyswapPairExtension for IBabyswapPair;
    using Tokens for IERC20;

    function calculateSwapReturn(
        IBabyswapFactory factory,
        IERC20 inToken,
        IERC20 outToken,
        ...
        IBabyswapPair pair = factory.getPair(realInToken, realOutToken);
        if (pair != IBabyswapPair(0)) {
